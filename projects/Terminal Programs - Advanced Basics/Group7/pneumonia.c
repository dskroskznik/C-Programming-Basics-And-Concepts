...
int main()
{
	...
	for(y=x-1;y>x-MAX_LEN;y--) { //condition statments to oly apply hospitals at certain echelons
		if(in[y].r >= 3) {
		if((strcmp(in[y].q, "Better")==0)||(strcmp(in[y].q, "Average")==0)) { //strcmp for cmparing letter values		
...
{
	struct hospitals t;//apply temporay struct case value
	int larg = 0;
	  if(n==1) {// for no infinite recursion
		return;}

	larg = find_largest(data, n);//applies find largest part func

	if(n-1 > larg) {//assorts members of data set by largest to smallest
		t = data[n-1];
		data[n-1] = data[larg];
		data[larg] = t;
	}
		sort_hospitals(data, n-1);//recalls func itself
}//end func

//integer function for locating largest section part of data set
int find_largest(struct hospitals data[], int n)
{
	int max_in = 0;//initialize max index
	int a;//array index 
	for(a=1;a<n;a++) {//find largest number(cost) within data
		if(data[a].ct > data[max_in].ct)
		   max_in = a;//reassigns a new cost value if larger
	} return max_in;
